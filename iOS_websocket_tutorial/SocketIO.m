...
- (void) connectToHost:(NSString *)host onPort:(NSInteger)port withParams:(NSDictionary *)params withNamespace:(NSString *)endpoint
{
    if (!_isConnected && !_isConnecting) {
        ...
        //NSURL *url = [NSURL URLWithString:@"ws://9900tipan_websocket.cloudfoundry.com:9900/socket.io/1/?t=16807"];
        // NSURL *url = [NSURL URLWithString:@"http://9900tipan_websocket.cloudfoundry.com:9900/socket.io/1/?t=16807"];
        query = nil;
...
- (void) onDisconnect:(NSError *)error
{
    DEBUGLOG(@"onDisconnect() with error: %@", error);
...
{ 	
    ...
    if ([_sid length] < 1 || [data count] < 4) {
        ...
        DEBUGLOG(@"did not receive valid data, possibly missing a useSecure?");
